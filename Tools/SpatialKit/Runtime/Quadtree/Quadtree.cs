...
namespace YokiFrame
{
    /// <summary>固定二维边界的四叉树空间索引，支持 XY/XZ 投影查询。</summary>
    public sealed partial class Quadtree<T> : ISpatialIndex<T>, ISpatialSnapshotProvider<T>
        where T : ISpatialEntity
    {
        ...
        private readonly Dictionary<int, QuadtreeNode> mEntityNodes = new Dictionary<int, QuadtreeNode>();
        private int mCount;

        /// <summary>创建固定二维边界的四叉树。</summary>
        /// <param name="bounds">必须为有限正尺寸的二维边界。</param>
        /// <param name="maxDepth">大于零的最大树深度。</param>
        /// <param name="maxEntitiesPerNode">大于零的单节点实体上限。</param>
        /// <param name="plane">实体位置使用的二维投影平面。</param>
        public Quadtree(
            YokiRect bounds,
        ...
        {
            SpatialMath.ValidateRect(bounds, nameof(bounds));
            ...
            mPlane = plane;
            mRoot = new QuadtreeNode(bounds, ROOT_DEPTH);
#if UNITY_EDITOR || (GODOT && TOOLS)
        ...
        public QuadtreeNode Root { get { return mRoot; } }
        ...
        public string IndexKind { get { return "Quadtree"; } }
        ...
        /// <summary>四叉树没有网格尺寸，固定返回零。</summary>
        ...
        /// <summary>表示四叉树不包含网格尺寸。</summary>
        public bool HasCellSize { get { return false; } }

        /// <summary>表示四叉树包含二维边界。</summary>
        public bool HasBounds2D { get { return true; } }

        /// <summary>表示四叉树不包含三维边界。</summary>
        public bool HasBounds3D { get { return false; } }

        /// <summary>获取二维根边界。</summary>
        public YokiRect Bounds2D { get { return mRoot.Bounds; } }

        /// <summary>四叉树没有三维边界，返回默认值。</summary>
        ...
        /// <summary>按 ID 从定位节点移除实体，不依赖实体当前可变位置。</summary>
        ...
        /// <summary>捕获当前四叉树的只读位置快照，供批量查询线程使用。</summary>
        ...
        public void QueryRadius(YokiVector3 center, float radius, List<T> results)
        {
            ...
            QueryRadiusNode(mRoot, center.X, centerB, radius, radius * radius, results);
        ...
        public void QueryBounds(YokiBounds bounds, List<T> results)
        {
            ...
            float minB = SpatialMath.GetPlaneCoordinate(bounds.Min, mPlane);
            float sizeB = SpatialMath.GetPlaneCoordinate(bounds.Size, mPlane);
            QueryBoundsNode(mRoot, new YokiRect(bounds.Min.X, minB, bounds.Size.X, sizeB), bounds, results);
        ...
        public T QueryNearest(YokiVector3 position, float maxDistance = float.MaxValue, Func<T, bool> filter = null)
        {
            ...
            QueryNearestNode(
                mRoot,
                position.X,
                SpatialMath.GetPlaneCoordinate(position, mPlane),
                position,
                filter,
                ref nearest,
                ref nearestDistanceSquared,
        ...
        /// <summary>清空实体并释放所有子节点，根节点恢复为叶节点。</summary>
        public void Clear()
        {
            ClearNode(mRoot);
            mEntities.Clear();
